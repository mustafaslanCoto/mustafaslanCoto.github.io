...
% Set IBM Plex Sans as the document font when using XeLaTeX/LuaLaTeX.
% Falls back to a sans-serif (Helvetica) when using pdfLaTeX.
\ifnum 0\ifxetex 1\fi\ifluatex 1\fi>0
  \usepackage{fontspec}
  \defaultfontfeatures{Ligatures=TeX}
  % Try to load bundled fonts from repo first (place font files in CV/fonts/)
  \IfFileExists{CV/fonts/IBMPlexSans-Regular.ttf}{%
    ...
    % Fallback to system-installed font name
    \setmainfont{IBM Plex Sans}[Ligatures=TeX]
    \setsansfont{IBM Plex Sans}[Ligatures=TeX]
  }
\else
  % pdfLaTeX fallback: use Helvetica-like sans-serif
...
$if(jobtitle)$\moveleft.5\hoffset\centerline{\textbf{\textcolor{black!80}{$jobtitle$}}}$endif$
...
\moveleft.5\hoffset\centerline{
  $if(email)$$if(fontawesome)$\faEnvelope \hspace{1 mm}$else$\emph{E-mail:}$endif$ \href{mailto:$email$}{\tt $email$} \hspace{1 mm}$endif$
  ...
  $if(linkedin)$$if(fontawesome)$\faLinkedin \hspace{1 mm}$else$\emph{LinkedIn:}$endif$ \href{https://linkedin.com/in/$linkedin$}{\tt $linkedin$} \hspace{1 mm}$endif$
  ...
  $if(kaggle)$$if(fontawesome)$\faKaggle \hspace{1 mm}$else$\emph{Kaggle:}$endif$ \href{http://kaggle.com/$kaggle$}{\tt $kaggle$} \hspace{1 mm}$endif$
